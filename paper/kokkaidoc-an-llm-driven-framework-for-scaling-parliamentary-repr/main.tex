...
\begin{frontmatter}
	...
	\affiliation[first]{organization={Divison of Engineering Science, University of Toronto}}
	\author[third]{Christopher Cochrane$^*$}
	\affiliation[third]{organization={Department of Political Science, University of Toronto Scarborough}}
	
	
	\begin{abstract}
	% Today we are living in a world where the application of LLMs in various fields are taking the world by storm. The ChatGPT platform made available by OpenAI made LLMs accessible to people without any STEM background and researchers in the field are able to download and share LLMs with the public. 
	% In this paper, we are proposing a new method to quantitatively express political disagreement from parliamentary speech data by leveraging LLM embeddings and applying statistical methods. Results show that the ideological positions of parliamentary representatives generated from our method is aligned with the general understanding of how the parties representatives are part of are positioned within various political topics. We also validate our findings by looking at political manifestos of the parties.
	
	% issues/notes: 
	% no mention of specific other methods vs our method (benchmark or contribution, like our method doesn't require training data for example, or maybe method makes it easy for everyone to use? we would need a package or user interface. kokkaidoc only allows people to view Japanese politics but not apply things on their own).  
	% wording of our project scope: is it that we're looking specifically at Japanese politics, or using Japanese politics as an example. not every coutnry wil have the same dataset and so it's not totally valid to say it's applicable to other political contexts since we don't at all do a comparative study. we can say that japanese politics is our current and only scope, and then further research is needed to establish its robustness to other contexts and available data
	% validation through repeated models 
	% provide 
...
\centering
  \centering
  \includegraphics[width=1\linewidth]{figs/data_collection_pipeline.pdf}
...
\centering
  \centering
  \includegraphics[width=1\linewidth]{figs/diachronic_projection.pdf}
...
\begin{figure}[h]
	\centering
	  \centering
	  \includegraphics[width=1\linewidth]{figs/diachronic_vis.pdf}
...
\centering
  \centering
  \includegraphics[width=0.8\linewidth]{figs/Segment_1.png}
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{figs/defence_diachronic.png}
...
\begin{figure}
	\centering
	  \centering
	  \includegraphics[width=\linewidth]{figs/nuclear_diachronic.png}
...
\begin{table}[h]
    ...
    \begin{tabular}{c|ccc}
        ...
        \hline
    \end{tabular}
    \caption{Number of Speech Segments / Number of Representatives on Nuclear Power}
...
\begin{CJK}{UTF8}{min}
    ...
    \begin{tabular}{c|ccc}
        ...
        \hline
    \end{tabular}
    \caption{Number of Speech Segments / Number of Representatives on Defense}
