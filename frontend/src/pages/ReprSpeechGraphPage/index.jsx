...
function ReprSpeechGraphPage() {
  ...
  useEffect(() => {
    ...
    axios({
      ...
      .then((res) => {
        ...
        setAvailableReprs(
          res.data['1d'].data.map(
            (d) => `${d.repr}-${d.party}-${House2House[d.house]}`
          )
        );
        setLineData(
  ...
  const pageContent = !Scatter2dData ? (
    ...
    <div className={`w-full ${isMobile ? 'h-full' : 'h-full'}`}>
      <div
        className={`${
      ...
      >
        <div className={`${isMobile ? 'w-full h-[50%]' : 'w-[70%]'} relative`}>
          {dimension === '1D' && showSpectrum && (
            ...
            >
              <LeftRightSpectrum
                leftLabel={currentCon}
                rightLabel={currentPro}
              />
            </div>
          )}
        ...
        <div className="flex h-full">
          ...
          >
            <h1 className="text-sm font-bold whitespace-nowrap">
              トピック:{Topic2Topic[currentTopic]}
            </h1>
            <h1 className="text-sm font-bold">
              対立軸:
              {
                currentAxisAvailability.find(
                  (axis) => axis.name === currentAxis
                )?.jpn
              }
            </h1>
          </div>
          <div
            className={`flex items-center justify-center w-full ${
              isMobile ? 'flex-row flex-1' : 'flex-col'
            }`}
          >
            {!isWebView && <TopicSelectionModalOpener addModal={addModal} />}
            <ReprSearchInput
              availableReprs={availableReprs}
              setCurrentRepr={setCurrentRepr}
              setCurrentParty={setCurrentParty}
              setCurrentHouse={setCurrentHouse}
            />
